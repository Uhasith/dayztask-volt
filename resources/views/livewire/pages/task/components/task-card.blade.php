<div x-init="initFlowbite();">
    <x-wui-card rounded="3xl" class="cursor-pointer" data-popover-target="popover-user-profile-{{ $task['uuid'] }}">
        <div class="flex items-center justify-between">
            <div class="max-w-[80%]">
                <p class="text-lg  truncate">{{ $task['name'] }}</p>
            </div>
                    ...
                    @elseif ($task['status'] == 'done')
                        <div wire:transition>
                            <x-wui-button 2xs orange label="Revert as todo"
                                x-tooltip.placement.top.raw="Revert as todo" />
                        </div>
                    @endif
        ...
        <div class="p-3">
            <div class="mb-4">
                @foreach ($task['users'] as $user)
                    @if ($user['id'] != auth()->user()->id)
                        <div class="flex items-center justify-between mb-2" wire:key="item-{{ $user['id'] }}">
                            ...
                            <div class="px-1">
                                <livewire:global.timer wire:key="timer-{{ $user['uuid'] }}" :trackedTime="$user['trackedTime']"
            ...
            <div class="my-2 px-1 flex items-center justify-between">
              ...
              <div class="my-2 px-1 flex items-center justify-between">
                ...
                <p class="font-semibold text-blue-600 dark:text-blue-500 ">{{ $task['deadline'] ? $task['deadline']->format('Y-m-d') : 'No Deadline' }}</p>
            </div>
