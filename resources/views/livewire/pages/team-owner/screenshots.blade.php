    ...
    <div class="mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-10 bg-teal-50 border border-teal-400 dark:bg-gray-800 dark:border-gray-800 p-5 rounded-lg">
            <div class="grid grid-cols-4 gap-5">
                ...
                <x-wui-select label="Filter by Tasks" placeholder="Select Tasks" wire:model.live="selectedTaskIDs"
                    multiselect :async-data="route('tasks.search')" option-label="name" option-value="id" />
                ...
                <div>
                    <x-wui-datetime-picker wire:model.live="selectedDate" label="Filter by Date"
                        placeholder="Select a Date" max="{{ Carbon::now()->format('Y-m-d') }}" without-timezone
                        without-time />
                </div>
            </div>
        </div>
        ...
        <ol class="relative border-s border-gray-200 dark:border-gray-700">
            ...
            <li class="mb-10 ms-4">
                ...
                <p class="mb-3 text-base font-semibold leading-none text-gray-800 dark:text-gray-500 uppercase">{{
                    Carbon::parse($hour . ':00')->format('h:i a') }} - {{ Carbon::parse($hour .
                    ':00')->addHours(1)->format('h:i a') }}</p>
                <div class="grid grid-cols-6 gap-5">
                    @foreach ($items as $screenshot)
                    <div class="media-item">
                        <div
                            class="border rounded-lg border-teal-200 px-5 py-3 bg-slate-50 dark:bg-gray-800 dark:border-gray-700">
                            <a data-fslightbox href="{{$screenshot->getUrl()}}" class="mb-4">
                                <img src="{{$screenshot->getUrl()}}" alt="Image"
                                    class="rounded-lg w-full h-32 object-cover" />
                            </a>
                            <div class="flex gap-3 items-center justify-between">

                                <a href="{{route('projects.tasks.update', $screenshot->model->uuid)}}"
                                    class="font-semibold text-gray-900 dark:text-white">{{$screenshot->model->name}}</a>

                                <div class="flex gap-1">
                                    <x-wui-icon data-tooltip-target="tooltip-{{$screenshot->id}}" name="clock"
                                        class="w-5 h-5" solid />
                                ...
                                <div id="tooltip-displays-{{$screenshot->id}}" role="tooltip"
                                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                    {{$screenshot->getCustomProperty('display_index') . ' of ' .
                                    $screenshot->getCustomProperty('display_count')}}
                                    <div class="tooltip-arrow" data-popper-arrow></div>
                                </div>
                            </div>
                            @if ($screenshot->getCustomProperty('user_id'))
                            <p class="text-xs mt-2 font-normal text-gray-500 dark:text-gray-400">
                                {{App\Models\User::find($screenshot->getCustomProperty('user_id'))->name}}</p>
                            @endif
                        </div>
                    </div>
                    @endforeach
